% Runs an Open Loop Random Noise Test which makes the user able to test
% an output or an input that is not directly connected to the 3670-A. 

addpath ..\;

% output plays the stimulus which gets captured by the inputs connected to the 3670-A
function externalOutput()
   ...
   res = engine.runCompleteOpenLoopRandomNoiseTest(1);
end
...
function externalInput()
   ...
   % The results from the test will be saved in the res variable. If the plots are active you have to close them before the function returns
   res = engine.runPlaybackOpenLoopRandomNoiseTest(1);
end
