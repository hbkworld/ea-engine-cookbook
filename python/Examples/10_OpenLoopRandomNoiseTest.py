...
# Runs an Open Loop Random Noise Test which makes the user able to test an output or an input that is not directly
...
def externalOutput():
    ...
    res = engine.RunCompleteOpenLoopRandomNoiseTest(1, triggerLevel=0.4, duration=10, lowerFrequency=20,
                                                    upperFrequency=20000,
                                                    burstSineLevel=0.8, burstSineFrequency=1000, burstSineCycles=1000,
                                                    level=0.5)
...
def externalInput():
    ...
    engine.setOutputChannel(1, "3670-A Ch.1", isActive=True)
    ...
    # The results from the test will be saved in the res variable. If the plots are active you have to close them before the function returns
    res = engine.RunPlaybackOpenLoopRandomNoiseTest(1, duration=10, lowerFrequency=20, upperFrequency=20e3)
